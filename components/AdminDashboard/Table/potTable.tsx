...
export default function PotTable({ pots, setPots  }: { pots: SinglePotInCard[], setPots: React.Dispatch<React.SetStateAction<SinglePotInCard[]>> }) {
      ...
      {isFormOpen ? (
        <PotForm
          ...
          onSubmit={async ({ finalUpdatePotData, newPotOptions, updatedPotOptions, deletedPotOptionIds }) => {
            ...
            await refresh();
            setPots(await getAllSinglePotInCard());
            setIsFormOpen(false);
            seteditingPotId("");
            setIsLoading(false);
          }
